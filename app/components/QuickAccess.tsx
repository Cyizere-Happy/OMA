...
const QuickAccess = ({ 
  ...
  const stats = { admissions: 14, triage: 3, beds: 12 };

  const statCards = [
    {
      label: "Admissions",
      value: `${stats.admissions} total`,
      sub: "Admitted patients",
      icon: <Building size={22} className="text-[#0B5B3E]" />,
    },
    {
      label: "Triage Queue",
      value: `${stats.triage} pending`,
      sub: "Awaiting evaluation",
      icon: <Flag size={22} className="text-[#0B5B3E]" />,
    },
    {
      label: "Ward Capacity",
      value: `${stats.beds} open`,
      sub: "Available beds",
      icon: <Wallet size={22} className="text-[#0B5B3E]" />,
    },
  ];
    ...
    <section className="px-10 py-8">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
        <div>
          ...
          <h2 className="text-3xl font-black text-stone-900 tracking-tight">Nursing Workspace</h2>
          <p className="text-[13px] text-stone-500 mt-2 max-w-xl">
            Admit patients, conduct triage, record vitals, and coordinate care securely.
          </p>
        </div>
        <button type="button" onClick={() => window.location.reload()}
          className="bg-white p-2 rounded-lg border border-stone-200 hover:bg-stone-50 transition-all group text-[#0B5B3E] shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {statCards.map((s) => (
          <div key={s.label} className="flex items-center gap-4 p-5 bg-white border border-stone-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
            <div className="p-3 rounded-xl bg-[#0B5B3E]/5 shrink-0">{s.icon}</div>
      ...
      <div className="flex items-stretch gap-4 p-5 bg-white border border-stone-200 rounded-2xl shadow-sm">
        <div className="p-3 rounded-xl bg-[#0B5B3E]/5 text-[#0B5B3E] shrink-0">
          <FileText size={20} />
        </div>
        <div className="flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <h4 className="text-[15px] font-black text-stone-900 mt-1">Nursing checklist</h4>
            <p className="text-[12px] text-stone-500 mt-0.5">Required vitals and tests before ward assignment.</p>
          </div>
          <button type="button" onClick={onOpenChecklist}
            className="self-start sm:self-center px-5 py-2.5 bg-[#0B5B3E] text-white text-[11px] font-bold rounded-xl hover:bg-[#2a4d75] transition-colors shrink-0">
            Open checklist
          </button>
        </div>
      </div>
